...
#ANTIJS_V2
#cl = LineClient
cl = LineClient(authToken='EEVBJdnAib9ENNTfwzH3.wuylwj4ToT26kXh9NU8+8W.UufDl6DZsWOZ7Nyu83gVA7O/3dDQG0ru/4UKyQ7MIso=')
cl.log("Auth Token : " + str(cl.authToken))
channel = LineChannel(cl)
cl.log("Channel Access Token : " + str(channel.channelAccessToken))

#ki = LineClient()
ki = LineClient(authToken='EER59i1P2Ol4lLSZU8Yd.cBqPBIPiNBYH4aWnNZR4Rq.EJlCBTgIH99L8ueqjrqjAFrcvpAGpjc/cNfxszrgKiI=')
ki.log("Auth Token : " + str(ki.authToken))
channel1 = LineChannel(ki)
ki.log("Channel Access Token : " + str(channel1.channelAccessToken))

#kk = LineClient()
kk = LineClient(authToken='EEA8LyoCGecsSAkBhTVc.4KIdIZKSYSOS6SooAeL7ta.TEQUyexAWY/KNIUfuIVDsff791aNzZ8BP05DiIVVV88=')
kk.log("Auth Token : " + str(kk.authToken))
channel2 = LineChannel(kk)
kk.log("Channel Access Token : " + str(channel2.channelAccessToken))

#kc = LineClient()
kc = LineClient(authToken='EE636j38hL0zI87B24Xf.WU1yVHpaPIBTBuD+dObI3W.P1gFZy9dr22/lw6wtFQenj05S4SHkTxW0T+t0fsD1BU=')
kc.log("Auth Token : " + str(kc.authToken))
channel3 = LineChannel(kc)
kc.log("Channel Access Token : " + str(channel3.channelAccessToken))

#sw = LineClient()
sw = LineClient(authToken='EEbwLGstTW4cFFoPGzC7.eLDo455gnmVoC175GjR4TW.t2658DXgzG7TV1hJaV4qFK2fEPj5LnA2zzqM/psSuzc=')
...
poll = LinePoll(cl)
call = cl
creator = ["u417afa40ff4bc387410cbc3531ce6853"]
owner = ["u417afa40ff4bc387410cbc3531ce6853"]
admin = ["u417afa40ff4bc387410cbc3531ce6853","uf158d35502ffaf2c7385fc83bba67821","u27612443166155c5b5b2497aa85b7f24","uf8a7a5f13031a917963bcbe285f9acbd","u9fdf3d45586d45582bb832fccc44aa3b","u5fbf1f1bb6a5fac8eb54eeac9a7ab611"]
staff = [""]
bod = [cl, ki, kk, kc, sw]
...
KAC = [cl,ki,kk,kc]
ABC = [cl,ki,kk,kc]
Bots = [owner,creator,admin,mid,Amid,Bmid,Cmid,Zmid]
all = owner + admin + creator + staff
Dpk = admin + staff
Setmain = {
    "keyCommand": "",
    "autoRead": {}
...
wait = {
    ...
    "welcome":"Welcome Gan,CEK NOTE YA",
    "comment":"Like like & like by Aχкѕαℓ",
...
def autosave():
        with open(datafile, "w") as x:
            json.dump(wait, x, sort_keys=True, indent=4,ensure_ascii=False)
               ...
               if msg.contentType == 0:
                    ...
                    if text is None:
                        ...
                        if cmd == "Bot on":
                          ...
                          if wait["selfbot"] == True:
                                cl.sendText(msg.to,"Creator Bot By") 
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
...
#____________                              
                        elif "Botupstatus " in msg.text:         
                          if msg._from in owner:
                            separate = msg.text.split("Botupstatus ")
                            string = msg.text.replace(separate[0] + "Botupstatus ","")
                          if len(string) <= 10000000000:
                            profile = cl.getProfile()
                            ...
                            profile.statusMessage= string
                            sw.updateProfile(profile)
                            cl.sendMessage(msg.to,"Status diganti jadi " + string + "")
#==============================================================================#
#===========BOT UPDATE============#
                        ...
                        elif msg.text.lower().startswith("add script"):
                            if msg._from in admin:
                                tRepl = msg.text.split("*")
                                try:namaFile = tRepl[1]
                                except:return cl.sendText(msg.to, "Value name file error, check again!")
                                try:script = tRepl[2]
                                except:return cl.sendText(msg.to, "Value script error, check again!")
                                try:f = open(namaFile, "a")
                                except:return cl.sendText(msg.to, "Nama file '" + namaFile + "' tidak ada.")
                                f.write("{}".format(script)); return cl.sendText(msg.to, "Success append script in file '" + namaFile + "', saved on your directory:)")
                            return
                        elif msg.text.lower().startswith("create file"):
                            if msg._from in admin:
                                tRepl = msg.text.split("*")
                                try:namaFile = tRepl[1].lower()
                                except:return cl.sendText(msg.to, "Value name file error, check again!")
                                try:script = tRepl[2]
                                except:return cl.sendText(msg.to, "Value script error, check again!")
                                f = open(namaFile, "w+"); f.write("{}".format(script)); return cl.sendText(msg.to, "Success create file '" + namaFile + "', saved on your directory:)")
                            return
                        elif msg.text.lower().startswith("read file"):
                            if msg._from in admin:
                                tRepl = msg.text.split("*")
                                namaFile = tRepl[1]
                                try:f = open(namaFile, "r");return cl.sendText(msg.to, str(f.read()))
                                except:return cl.sendText(msg.to, "File name '" + namaFile + "' doesn't exist!")
                            return
                        elif cmd == "listbot":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to,"List  Bot\n\n"+ma+"\nTotal「%s」 Bots" %(str(len(Bots))))
                          ...
                          if wait["selfbot"] == True:
                                ...
                                for m_id in owner:
                                    ...
                                    ma += str(a) + ". " +cl.getContact(m_id).displayName + "\n"
                                cl.sendMessage(msg.to,"List  Owner\n\n"+ma+"\nTotal「%s」 Owner" %(str(len(owner))))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                cl.sendMessage(msg.to,"Harap copy text di bawah ini")
                                cl.sendMessage(msg.to,"Just Ganti Nomor Group")
                                cl.sendMessage(msg.to,"!!!exec\ncl.inviteGroupNumber(msg._from,Nomor Group)\ncl.sendText(msg.to,Sucsess Invite Group")
                                cl.sendText(msg.to,"Sucsess Invite Group")

                        elif cmd == "invitelinkmeto":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                cl.sendMessage(msg.to,"Harap copy text di bawah ini")
                                cl.sendMessage(msg.to,"!!!exec""\n\cl.inviteGroupNumberByLink(msg._from,nomor group)")

                        elif cmd == "respon":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                wait["qr"] = False
                                cl.sendMessage(msg.to,"Respon Start...")
                                cl.sendMessage(msg.to,responsename0)
                                ki.sendMessage(msg.to,responsename1)
                                kk.sendMessage(msg.to,responsename2)
                                kc.sendMessage(msg.to,responsename3)
                                sw.sendMessage(msg.to,responsename4)
                                cl.sendMessage(msg.to,"Respon Selesai...")
...
#---------------------Sc invite owner ke group------
                        elif msg.text.lower().startswith("invitemeto:"):
                            if msg._from in admin or msg._from in owner or msg._from in creator:
                                s = msg.text.replace("invitemeto: ", "")
                                bod[0].inviteGroupNumber(msg._from, s)
                                bod[0].sendText(msg.to, "Success Invite To" +str(ginfo.name))
							
                        elif msg.text.lower().startswith("invitelinkmeto"):
                            if msg._from in admin or msg._from in owner or msg._from in creator:
                                s = msg.text.replace("invitelinkmeto ", "")
                                bod[0].inviteGroupNumberByLink(msg._from, s)
                                return bod[0].sendText(msg.to, "Success Send Link, Cek Chat" +str(ginfo.name))
#--------===---====--------------
                        elif msg.text.lower() == "add admin gw":
                            if msg._from not in admin:admin.append(msg._from); return cl.sendText(msg.to, "Ok!")
                            return cl.sendText(msg.to, "Dancok udah!")
                        elif msg.text.lower() == "add owner gw":
                            if msg._from not in owner:owner.append(msg._from); return cl.sendText(msg.to, "Ok!")
                            return cl.sendText(msg.to, "Dancok udah!")
                        elif msg.text.lower().startswith("inviteall"):
                            if msg._from in admin or msg._from in owner or msg._from in creator:
                                tSplit = msg.text.split(":")
                                try: float(int(tSplit[1]))
                                except: return cl.sendText(msg.to, "Invalid selected bot number, please choose [1/2/3/4]!")
                                botAksal = [cl, ki, kk, kc] 
                                if tSplit[1] == "1":return botAksal[0].inviteIntoGroup(msg.to, admin)
                                if tSplit[1] == "2":return botAksal[1].inviteIntoGroup(msg.to, admin)
                                if tSplit[1] == "3":return botAksal[2].inviteIntoGroup(msg.to, admin)
                                if tSplit[1] == "4":return botAksal[3].inviteIntoGroup(msg.to, admin)
                                return botAksal[0].sendText(msg.to, "Invalid choosing command '" + tSplit + "', please choose [1/2/3/4]!")
                            return																							
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                ...
                                for i in gid:
                                    ...
                                    if h == ng:
                                        cl.sendMessage(i, "Silahkan admin invite atau masukan kembali")
                                        cl.leaveGroup(i)
                                        ki.leaveGroup(i)
                                        kk.leaveGroup(i)
                                        kc.leaveGroup(i)
...
#=================+
                            ...
                            try:
                                ...
                                cl.sendText(msg.to,"PEMBUAT GRUP INI") 
                            except:
...
#===========Set===================#
                           ...
                           if msg._from in admin:
                            ...
                            if msg._from in admin:
                                ...
                                random.choice(ABC).sendMessage(msg.to,"All Protect Turned On")
                            else:
                                random.choice(ABC).sendMessage(msg.to,"Just for Admin")
                            ...
                            if msg._from in admin:
                                ...
                                random.choice(ABC).sendMessage(msg.to,"All Protect Turned Off")
                            else:
                                random.choice(ABC).sendMessage(msg.to,"Just for Admin")    
...
#===========KICKOUT============#
                          ...
                          if wait["selfbot"] == True:
                            ...
                            #print "ok"
                                ...
                                if targets == []:
                                    random.choice(ABC).sendMessage(msg.to,"Target Ga Ada")
                                else:
                                    for target in targets:
                                        if not target in admin and not target in bots:
                                            try:
                                      ...
                                      if not target in Bots:                    
                                              if not target in admin:
                                                  try:
                                                      ...
                                                      bod[0].sendMessage(msg.to,"Sucsess") 
...
#===========ADMIN ADD============#
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in creator:
                               ...
                               for target in targets:
                                   if target not in Dpk:
                                       try:
                                           owner.remove(target)
                                           cl.sendMessage(msg.to,"Berhasil menghapus owner")
                                       except:
                                           pass
                            ...
                            if msg._from in owner:
                                wait["addadmin"] = False
                                cl.sendText(msg.to,"Kirim kontak dinonaktifkan...")
                        ...
                        elif cmd == "contact creator" or text.lower() == 'contact owner':
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
                                    cl.sendMessage(msg.to, None, contentMetadata={'mid': i}, contentType=13)
...
#===========COMMAND BLACKLIST============#
                        ...
                        elif cmd == "untalkban:off" or text.lower() == 'untalkban:off':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                wait["Talkdblacklist"] = False
                                cl.sendText(msg.to,"Kirim kontak dinonaktifkan...")
                            ...
                            if msg._from in admin:
                              if wait["Talkblacklist"] == {}:
                                ...
                                for m_id in wait["Talkblacklist"]:
                                    ...
                                    ma += str(a) + ". " +cl.getContact(m_id).displayName + "\n"
                                cl.sendMessage(msg.to,"List Talkban User\n\n"+ma+"\nTotal「%s」Talkban User" %(str(len(wait["Talkblacklist"]))))
